...
const RestaStock = () => {
  ...
  const restarStock = () => {
    axios.post('http://localhost:5050/restar', {
        Cproducto,
        cantidad
      })
      .then((response) => {
        // Maneja la respuesta exitosa
        navigate('/mensaje', { state: { mensaje: '¡Stock restado con éxito!' } });
    })
    .catch((error) => {
        // Maneja los errores
        navigate('/mensaje', { state: { mensaje: 'Error al restar el stock: ', error } });
    });
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.text}>RESTAR STOCK </Text>
      ...
      />
      <Pressable style={[styles.pressableButton, { alignSelf: 'center' }]} onPress={restarStock}>
